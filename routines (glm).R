...
kfoldCV.mle <- function(model,data,K=10,seed) {
...
## - pred: cross-validated predictions for y
  ...
  subset <- rep(1:K,ceiling(nrow(data)/K))[1:nrow(data)]
  subset <- sample(subset,size=nrow(data),replace=FALSE)
  pred <- double(nrow(data))
  if (ncol(data)>0) {
    for (k in 1:K) {
      sel <- subset==k
      fit <- lm(model, data[!sel,,drop=FALSE])
      pred[sel] <- predict(fit, newdata=data[sel,,drop = FALSE])
    }
  } else {print('you gave me no data!')}
...
kfoldCV.lasso <- function(y,x,K=10,seed,criterion='cv', s = 'lambda.min') {
...
kfoldCV.lasso.glm <- function(y,x,K=10,seed,criterion='cv', family = 'gaussian', predict_type = "link", s = 'lambda.1se') {
  ## Perform K-fold cross-validation for LASSO regression estimate (lambda set either via cross-val or BIC or EBIC)
  ## Input
  ## - y: response
  ## - x: data.frame with predictors, intercept should not be present
  ...
  if (ncol(x)>0) {  #if there are some covariates
    for (k in 1:K) {
      ...
      if (criterion=='cv') {
        ...
        pred[sel] <- predict(fit, newx= x[sel,,drop=FALSE], type = predict_type,  s = s) 
      } else if (criterion=='bic'){
        ...
        pred[sel] <- predict(model, newx= x[sel,,drop=FALSE], type = predict_type,  s = s)
      } else if (criterion=='ebic'){
        fit <- lasso.bic.glm(y=y[!sel],x=x[!sel,,drop=FALSE], family = family, extended = TRUE)
        model <- glmnet(x=x[!sel,,drop=FALSE],y=y[!sel],lambda = fit$lambda.opt,family=family,s = s)
        pred[sel] <- predict(model, newx= x[sel,,drop=FALSE], type = predict_type,  s = s)
      } else { stop("method.lambda not implemented") }
      cat(".")
    }
...
kfoldCV.bayes <- function(y,x,K=10,seed,
  ...
  if (ncol(x)>0) {
    for (k in 1:K) {
      sel <- subset==k
      fit <- modelSelection(y = y[!sel], x = x[!sel,,drop=FALSE], priorCoef = priorCoef,
                            priorDelta = priorDelta)
      # Need to compute predictions manually, no method I could find within msfit object (predict is not actually for predictions)
      pred[sel] <- cbind(1,x[sel,,drop=FALSE]) %*% coef(fit)[,1][-length(coef(fit)[,1])]
    }
  } else {
    for (k in 1:K) {
      sel <- subset==k
...
kfoldCV.qr <- function(model, data ,K=10,seed,tau=0.5, method = "lasso", predict_type = "link") {
  ## Perform K-fold cross-validation for quantreg estimate
  require(quantreg)
  if (!missing(seed)) set.seed(seed)
  subset <- rep(1:K,ceiling(nrow(data)/K))[1:nrow(data)]
  subset <- sample(subset,size=nrow(data),replace=FALSE)
  pred <- double(nrow(data))
  cat("Starting cross-validation")
  if (ncol(data)>0) {  #if there are some covariates
    for (k in 1:K) {
      sel <- subset==k
      fit <- rq(formula = model, data = data[!sel,,drop=FALSE], tau = tau, method = method)
      pred[sel] <- predict.rq(fit, newdata= data[sel,,drop=FALSE], type = predict_type) 
      cat(".")
    }
  } else { 
  cat("You haven't given me any data!!")
    }
  cat("\n")
  return(list(pred=pred))
}

kfoldCV.qrbayes <- function(model, data ,K=10,seed,quantile=0.5, alasso = TRUE,
                            normal.approx = FALSE, ndraw = 5000, burnin = 1000) {
  ## Perform K-fold cross-validation for quantreg estimate
  require(bayesQR)
  if (!missing(seed)) set.seed(seed)
  subset <- rep(1:K,ceiling(nrow(data)/K))[1:nrow(data)]
  ...
  if (ncol(data)>0) {  #if there are some covariates
    for (k in 1:K) {
      sel <- subset==k
      fit <- bayesQR(model, data[!sel,,drop=FALSE], quantile = quantile, alasso = alasso, normal.approx = normal.approx, ndraw = ndraw)
      pred[sel] <- predict.bayesQR(fit, newdata= data[sel,,drop=FALSE], burnin = burnin) 
      cat(".")
    }
  } else { 
    cat("You haven't given me any data!!")
...
kfoldCV.lmer <- function(model, data ,K=10,seed) {
  ## Perform K-fold cross-validation for quantreg estimate
  require(lmerTest)
  if (!missing(seed)) set.seed(seed)
  subset <- rep(1:K,ceiling(nrow(data)/K))[1:nrow(data)]
  ...
  if (ncol(data)>0) {  #if there are some covariates
    for (k in 1:K) {
      sel <- subset==k
      fit <- lmerTest::lmer(model, data[!sel,,drop=FALSE])
      pred[sel] <- predict(fit, newdata= data[sel,,drop=FALSE], allow.new.levels = TRUE) 
      cat(".")
    }
  } else { 
    cat("You haven't given me any data!!")
...
lassopost  <- function(y,x, method.lambda='bic', verbose = FALSE, family = 'gaussian') {
  ...
  #Set lambda
  if (method.lambda == 'cv') {
    cvfit= cv.glmnet(x=xstd, y=ystd, nfolds=10, family = family)
    lambda= cvfit$lambda.1se
  } else if (method.lambda == 'bic') {
    lambda= lasso.bic(y=ystd, x=xstd, extended = FALSE)$lambda.opt
  } else { stop("method.lambda not implemented") }
  #LASSO estimate for selected lambda            
  gfit= glmnet(x=xstd, y=ystd, standardize=FALSE, lambda=lambda, family = family)
  b= coef(gfit)[-1]
  #Post-selection inference
  lcv= fixedLassoInf(x=xstd,y=ystd,beta=b,lambda=lambda*nrow(x),sigma=sigmahat,verbose=verbose, family = family)
  sel.lasso= (b!=0)
